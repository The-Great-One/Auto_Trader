# Options Research Supervisor, 2026-04-21

- Market open: **True** (NSE calendar)
- Fetch ran: **True**
- Paper shadow ran: **True**
- Paper shadow stale: **False**
- Options lab ran: **True**
- Options lab stale: **False**

## Daily options iteration plan
- [medium] **broaden_options_search**: Daily options lab did not improve on baseline. Broaden the search across underlying trend filters, score thresholds, and exit logic.
- [medium] **deeper_options_weekend_sweep**: Latest options lab tested 0 variants. Keep daily runs lean, but use deeper weekend sweeps for better separation.
- [medium] **add_options_near_miss_diagnostics**: Top options paper candidate is still HOLD with score 6.0. Add gate-by-gate miss diagnostics so daily iteration can tune the last blockers.
- Contracts selected: **28**
- Contracts fetched: **28**
- Contracts failed: **0**
- Options evaluated: **28** / **28**
- BUY candidates now: **0**
- Top candidate: **NIFTY26APR24550CE**
- Top decision: **HOLD**
- Top score: **6.0**
- Lab variants tested: **None**
- Lab best: **n/a**
- Lab best return %: **None**
- Lab improvement % vs baseline: **None**

## Paper shadow stderr
```
[2026-04-21 15:50:18,609] {RULE_SET_2.py handle_sell:210 MainThread} INFO - Removed NIFTYETF from stop-loss JSON after selling

```
