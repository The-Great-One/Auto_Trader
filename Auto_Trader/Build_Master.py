...
def create_master(message_queue):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "Fetched instrument master, holdings, and good stock lists successfully."
            )
        ...
        try:
            ...
            logger.error(
                f"Error renaming columns in holdings: {str(e)}\n{traceback.format_exc()}"
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.error(
                f"Error mapping instrument tokens: {str(e)}\n{traceback.format_exc()}"
            )
            sys.exit(1)
        ...
        try:
            ...
            if "ETFTheme" not in mapped_df.columns:
                mapped_df["ETFTheme"] = ""
            mapped_df["ETFTheme"] = mapped_df["ETFTheme"].fillna("")
            logger.info("Merged holdings with mapped data and removed duplicates.")
        except Exception as e:
            logger.error(
                f"Error merging holdings data: {str(e)}\n{traceback.format_exc()}"
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.error(
                f"Error downloading historical quotes: {str(e)}\n{traceback.format_exc()}"
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.error(
                f"Error processing fetched files: {str(e)}\n{traceback.format_exc()}"
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.error(
                f"Error merging data and saving CSV: {str(e)}\n{traceback.format_exc()}"
            )
            sys.exit(1)
