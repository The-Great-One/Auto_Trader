...
def buy_or_sell(df, row, holdings):
   ...
   # --- Buy Conditions ---
    ...
    sell_liquidity_condition = (
        (latest_data['Close'] * latest_data['Volume'] > 5000000) and
        (latest_data['Close'] * latest_data['Volume'] >
         latest_data['SMA_20_Close'] * latest_data['SMA_20_Volume'])
    )
    ...
    sell_any_condition = (
        (previous_data['Close'] >= previous_data['SMA_20_Close'] and latest_data['Close'] < latest_data['SMA_20_Close']) or
        (previous_data['RSI'] >= 40 and latest_data['RSI'] < 40) or
        (previous_data['Close'] >= previous_data['Supertrend'] and latest_data['Close'] < latest_data['Supertrend'])
    )
